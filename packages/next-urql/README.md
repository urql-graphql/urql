...
When you are using `getStaticProps`, `getServerSideProps`, or `getStaticPaths`, you should opt-out of `Suspense` by setting the `suspense` option to `false` in your `withUrqlClient` configuration.
your `withUrqlClient`.
