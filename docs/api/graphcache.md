...
| writeMetadata | `(json: SerializedRequest[]) => void`         | This provided method must be able to persist met-data for the cache. For backwards compatibility it should be able to accept any JSON data.                                             |
| readMetadata  | `() => Promise<null | SerializedRequest[]>`   | This provided method must be able to read the persisted met-data that has previously been written using `writeMetadata`. It's only called on startup.                                   |
